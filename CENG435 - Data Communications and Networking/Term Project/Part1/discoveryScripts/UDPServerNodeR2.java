...
/* Main class which executes main Server thread for Node R2.*/
public class UDPServerNodeR2 {

    public static void main(String args[]) {

        DatagramSocket serverSocket = null;
        try {
            serverSocket = new DatagramSocket(1070);
            byte[] receiveData = new byte[2000];
            byte[] sendData = new byte[2000];
            boolean isMessagingStarted = false;

            int messageCount = 1;
            System.out.println("Server on Node R2 starts to listen...");

            /* It expects total 4000 messages from negihbour S(1000 messages)-R1(1000 messages)-D(1000 messages)-R3(1000 messages) clients. 
        	Receive method of DatagramSocket is blocking.
        	Send method is just sending "ACK" message to sender client back. */
            while (messageCount <= 4000) {
                ...
                /* Node R2 starts its sender Clients for each of its neighbour node's servers. */
                if (isMessagingStarted == false) {
                    Client UDPClientForNodeS = new Client("R2","S","s.61tp.ch-geni-net.instageni.cenic.net",1050);
                    Thread UDPClientThreadForNodeS = new Thread(UDPClientForNodeS);

                    Client UDPClientForNodeR1 = new Client("R2","R1","r1.61tp.ch-geni-net.instageni.cenic.net",1060);
                    Thread UDPClientThreadForNodeR1 = new Thread(UDPClientForNodeR1);

                    Client UDPClientForNodeR3 = new Client("R2","R3","r3.61tp.ch-geni-net.instageni.cenic.net",1080);
                    Thread UDPClientThreadForNodeR3 = new Thread(UDPClientForNodeR3);

                    Client UDPClientForNodeD = new Client("R2","D","d.61tp.ch-geni-net.instageni.cenic.net",1090);
...
/* Sender Client thread implementation for Node R2 */
    ...
    public void run() {
        ...
        try {
            ...
            /* Averaging RTT values and creating link_cost(X-Y).txt files. 
              If destination is not R1 server(port=1060), do this because link cost of (R1-R2) is estimated and saved by R1 node(at client side). */
            if (destinationPort != 1060) {
            	long sum = 0;
