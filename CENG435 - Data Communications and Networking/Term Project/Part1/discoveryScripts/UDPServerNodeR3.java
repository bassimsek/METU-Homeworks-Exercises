...
/* Main class which executes main Server thread for Node R3.*/
public class UDPServerNodeR3 {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            while (messageCount <= 3000) {
                ...
                /* Node R3 starts its sender Clients for each of its neighbour node's servers. */
                if (isMessagingStarted == false) {
                    Client UDPClientForNodeS = new Client("R3","S","s.61tp.ch-geni-net.instageni.cenic.net",1050);
                    Thread UDPClientThreadForNodeS = new Thread(UDPClientForNodeS);

                    Client UDPClientForNodeR2 = new Client("R3","R2","r2.61tp.ch-geni-net.instageni.cenic.net",1070);
                    Thread UDPClientThreadForNodeR2 = new Thread(UDPClientForNodeR2);

                    Client UDPClientForNodeD = new Client("R3","D","d.61tp.ch-geni-net.instageni.cenic.net",1090);
...
/* Sender Client thread implementation for Node R3 */
    ...
    public void run() {
        ...
        try {
        	...
        	/* Averaging RTT values and creating link_cost(X-Y).txt files. 
              If destination is S server(port=1050) or D server(port=1090), do this because link cost of (R3-R2) is estimated and saved by R2 node(at client side). */
            if (destinationPort == 1050 || destinationPort == 1090) {
	            long sum = 0;
